...
import { apiPatchInterval, apiPostLogin, apiPostRegister } from "@Interval/api/api";
import { IntervalIntensityType } from "@Interval/api/utils/apiTypes";
import { loginErrorState, registerErrorState } from "@Interval/serverAction/utils/serverActionConstants";
...
export const apiPostLoginServerAction = async (formData: FormData) => {
    ...
    try {
        const loginResponse = await apiPostLogin({
            email,
            password,
        });
...
export const apiPostRegisterServerAction = async (formData: FormData) => {
    ...
    if (passwordValidationError) {
        registerErrorState.setPasswordError(passwordValidationError as RegisterError);
        revalidatePath("/");
