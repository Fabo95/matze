...
export const SliderTrack = ({
    configurationType,
    sliderRange,
}: Pick<IntervalTimerConfigurationOptionProps, "configurationType" | "sliderRange">) => {
    // --- CALLBACKS ---
        ...
        () =>
            getArrayWithElements({
                ...
                .map((_, dividingLineIndex) => {
                    const dividingTimeClassName = dividingLineIndex % 6 === 0 && "slider-track-dividing-line-long";
                    const dividingCountClassName = dividingLineIndex % 5 === 0 && "slider-track-dividing-line-long";
                    ...
                    const invertedDividingLineIndex = Math.abs(dividingLineIndex - totalDividingLines);

                    const dividingLineLabel = (invertedDividingLineIndex / totalDividingLines) * sliderRange.to;

                    return (
                        <Box className="slider-track-dividing-line-container" key={uuidv4()}>
                            {dividingLineClassName && (
                                <Box className="slider-track-dividing-line-label">{dividingLineLabel}</Box>
                            )}
                            <Box className={`${dividingLineClassName} slider-track-dividing-line`} />
                        </Box>
                    );
                }),
