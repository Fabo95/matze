...
class AnimationController {
    ...
    initializeAnimation(keyframes: Keyframe[], timing: number | KeyframeAnimationOptions) {
...
export class PulseAnimation {
    ...
    executeReverseStartPulsing() {
        ...
        this.animationControllers.forEach((controller) => {
            const iterationProgress = controller.animation?.effect?.getComputedTiming().progress;

            if (iterationProgress && (!smallestIterationProgress || iterationProgress < smallestIterationProgress)) {
                smallestIterationProgress = iterationProgress;
            }
        });

        const smallestIterationAnimationTime = (smallestIterationProgress || 0) * PULSE_ANIMATION_DURATION;
        ...
        this.animationControllers.forEach((controller) => {
            ...
            if (controller.state === AnimationPlayState.FINISHED) {
                ...
                controller.animation?.effect?.updateTiming({
                    delay: Math.abs(smallestIterationAnimationTime - animationDistanceInMs),
                });
            }
    ...
    executeStopPulsing() {
        this.animationControllers.forEach((controller) => {
            ...
            const iterationAnimationTime = (iterationProgress || 0) * PULSE_ANIMATION_DURATION;
