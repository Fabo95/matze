...
import { AnimationPlayState, PulseAnimationProps } from "@Interval/components/core/pulse/utils/pulseTypes";
...
export const Pulse = ({ isAnimating }: PulseProps) => {
    ...
    const isAnimationsPlayState = useCallback((playState: AnimationPlayState) => {
        ...
        if (playState === AnimationPlayState.REVERSING) {
            return pulseAnimation.animationControllers.some((controller) => controller.state === playState);
        }

        return pulseAnimation.animationControllers.every((controller) => controller.state === playState);
    }, []);
